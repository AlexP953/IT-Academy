...
// Nivell 3 Exercici 1 - Criba d'Eratòstenes 

function eratostenesCribe(int $number): void {
  $numbersToCribe = [];
  $primeNumbers = [];

  // Aqui añado a un array todos los numeros desde 2 hasta $number
  for ($i = 2; $i <= $number; $i++) { 
    $numbersToCribe[] = $i;
  }

  while (count($numbersToCribe) > 0) {
    // Mientras numbersToCribe tenga contenido, se hara este bucle
    // Pilla el primer numero del array y lo añade a la lista de numeros primos.
    $primeNumber = $numbersToCribe[0];
    $primeNumbers[] = $primeNumber;

    // Con el primer numero del array, filtra los numeros multiples por el mismo
    $numbersToCribe = array_filter($numbersToCribe, function($numb) use ($primeNumber) {
      return $numb % $primeNumber !== 0 || $numb === $primeNumber;
    });

    // Y elimina el primer numero del array porque ya está como numero primo.
    array_shift($numbersToCribe);
  }
